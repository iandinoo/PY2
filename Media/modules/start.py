...
Silahkan Lakukan Pembayaran Yang Sudah Ditentukan.
"""
...
async def generate_unique_ref_id():
    return 'SMx' + uuid.uuid4().hex[:16].upper()
...
async def start(client, message):
    ...
    if not await get_maintenance():
        return await message.reply("🛠️ BOT SEDANG PERBAIKAN - KEMBALI LAGI NANTI")
    try:
        start_button = InlineKeyboardButton("BERGABUNG", callback_data="create_qris")
        ...
        if response.status_code == 200:
            ...
            if data["status"] == "success":
                ...
                if image_response.status_code == 200:
                    ...
                    success_button = InlineKeyboardButton("✅ Sudah Membayar", callback_data=f"transaction_{final_amount}_{pay_invoice}")
                    ...
                    qris_message = await cb.message.reply_photo(photo=bio, caption=C10.format(pay_invoice, random_addition, final_amount_str), reply_markup=start_markup)
...
async def cancel(client, cb):
    ...
    await delete_pending_transaction(user_id)
    await cb.message.reply("<b>❌ Pembayaran Dibatalkan.</b>")
...
async def create_logger_link(cb, pay_invoice):
    ...
    try:
        message = await bot.send_message(
            chat_id=chat.id,
            text=(
                f"<b>🆕 Pengguna Membeli Link!</b>\n\n"
                f"<b>👤 Nama:</b> {mention}\n"
                f"<b>💰 Total Harga:</b> {nominal}\n"
                f"<b>🆔 Pay Invoice:</b> `{pay_invoice}`\n"
                f"<b>📅 {DATE} {TIME}</b> WIB"
            ),
            reply_markup=InlineKeyboardMarkup([[view_profile_button]])
        )
    except UserIsBlocked:
        LOGGER("INFO").info(f"⛔ UserIsBlocked: {pay_invoice}")
        return
