...
async def generate_unique_ref_id():
    return 'YS' + uuid.uuid4().hex[:16].upper()
...
async def cancel(client, cb):
    user_id = cb.from_user.id
    await cb.message.delete()
    LOGGER("INFO").info(f"❌ Pembaran Dibatalkan.")
    await delete_pending_transaction(user_id)
    await cb.message.reply("<b>❌ Pembayaran batalkan.</b>")

@bot.on_message(filters.command("cancel") & filters.private)
async def cancel(client, message):
    user_id = message.from_user.id
    await delete_pending_transaction(user_id)
    await message.reply("<b>❌ Pembayaran Dibatan.</b>")
